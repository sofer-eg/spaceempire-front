...
function CreateClanForm({ onCreated }: { onCreated: () => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      //
      // What comes back is the sentinel's own text, which the handler answers 409
      // with verbatim (err.Error(), clans/server.go writeServiceError) and which
      // friendlyError passes through unchanged for a non-502/504 ApiError. That
      // used to be English with the package name in front of it — «clans: name
      // already taken» on a Russian screen. TASK-185 Russianised the fourteen
      // sentinels in clans/errors.go and dropped the prefix, so «клан с таким
      // названием уже есть» is what a collision reads as; no mapping is needed
      // here, and adding one would only put a second wording in play.
      setError(friendlyError(e));
    } finally {
      setBusy(false);
    }
